...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    __shared__ unsigned char gg[38400];
    ...
    for (int x = threadIdx.x; x < width; x+=blockDim.x){
        for (int v = -yBound; v <= yBound; ++v) {
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    gg[((v + 2) * 5 + u + 2) * 3 + 75 * threadIdx.x + 2] = s[channels * (width * (y + v) + (x + u)) + 2];
                    gg[((v + 2) * 5 + u + 2) * 3 + 75 * threadIdx.x + 1] = s[channels * (width * (y + v) + (x + u)) + 1];
                    gg[((v + 2) * 5 + u + 2) * 3 + 75 * threadIdx.x + 0] = s[channels * (width * (y + v) + (x + u)) + 0];
                }
            }
        }
        // __syncthreads();
        ...
        for (int i = 0; i < Z; ++i) {
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                for (int u = -xBound; u <= xBound; ++u) {
                    if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                        const unsigned char R = gg[((v + 2) * 5 + u + 2) * 3 + 2 + 75 * threadIdx.x];
                        const unsigned char G = gg[((v + 2) * 5 + u + 2) * 3 + 1 + 75 * threadIdx.x];
                        const unsigned char B = gg[((v + 2) * 5 + u + 2) * 3 + 0 + 75 * threadIdx.x];
