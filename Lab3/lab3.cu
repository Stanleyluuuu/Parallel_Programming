...
__global__ void sobel_cuda (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    // calculate the starting and ending point for each thread
    start = width / 1024 * threadIdx.x;
    end = width / 1024 * (threadIdx.x + 1);
    if (width % 1024 != 0){
        if (threadIdx.x < width % 1024){
            start += threadIdx.x;
            end += (threadIdx.x + 1);
        }
        else{
    ...
    y = blockIdx.x;
    if (y < height){
    // for (y = blockIdx.x; y < height; y+=blockDim.x) {
        // for (x = threadIdx.x; x < width; x+=1024){
        for (x = start; x < end; x++){
            for (i = 0; i < MASK_N; ++i) {
                adjustX = (MASK_X % 2) ? 1 : 0;
                adjustY = (MASK_Y % 2) ? 1 : 0;
...
int main(int argc, char** argv) {
    ...
    cudaMemcpy(device_t, host_t, height * width * channels * sizeof(unsigned char), cudaMemcpyHostToDevice);

    /* Hint 3 */
    // acclerate this function
    sobel_cuda<<<15000, 1024>>>(device_s, device_t, height, width, channels);
...
// nvcc -lpng lab3.cu -o lab3
// srun -n 1 --gres=gpu:2 ./lab3 ./testcases/candy.png ./mine/candy.out.png